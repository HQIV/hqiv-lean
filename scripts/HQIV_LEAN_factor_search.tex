...
\subsection{Hierarchical harmonic step budget}

The monolithic driver uses a derived per-level step budget for mod-trace length
and age thresholds. Let $b$ be the bit length of the odd core after peeling powers
of two, and let $L$ be the current promotion level (number of hierarchical
refinements performed). The budget is

\[
\texttt{harmonicStepBudget}(b,L) = \max\bigl(4,\ 
\bigl\lfloor (b-1)^{1+1/b} / (1 + 0.8L) \bigr\rfloor \bigr)
\]

with an optional credit of 8 bits when small primes ($\le 257$) are pre-peeled.

\textbf{Derivation.} After peeling 2's the remaining uncertainty in factor location
is $b/2$ bits. Each promotion level $L$ performs an $(L+2)$-section of the most
promising angular interval, reducing uncertainty by $\sum_{k=0}^L \log_2(k+2)$ bits.
The base $(b-1)^{1+1/b}$ is the information-theoretic cost of resolving the initial
uncertainty; division by $(1+0.8L)$ encodes harmonic decay (finer resolution requires
proportionally less work per register to certify periodicity). The floor of 4 prevents
premature termination at very deep levels. Pre-peeling small primes reduces effective
$b$ by $\approx 8$ bits, directly lowering the budget at every level.

This budget is used both for the per-register mod-trace cap and (via the age
threshold) for deciding when to promote a register to the next resolution level.
