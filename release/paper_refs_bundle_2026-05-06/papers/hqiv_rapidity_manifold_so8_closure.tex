...
% Reproducibility: this manuscript and the artifact bundle share one Zenodo deposit.
% Replace the placeholder DOI with the published record before distribution.
\newcommand{\PaperZenodoDoi}{10.5281/zenodo.XXXXXXXX}
% Use \path (underscore-safe) for bundle paths; do not wrap in \texttt.
...
Scripts, JSON certificates, Lean modules, and both PDF/\TeX{} sources cited below for audit are collected under directory \PaperArtifactBundleDir/ inside archive \PaperArtifactBundleZip, uploaded to the \emph{same} Zenodo record as this manuscript: \PaperZenodoDeposit.
After unpacking, filesystem paths in this paper are relative to \PaperArtifactBundleDir/ (see \path{MANIFEST.json}).
...
\medskip
\noindent\textbf{Caveat (holonomy vs.\ full closure).} Theorem~\ref{thm:universal-holonomy} below is a planar holonomy statement for an embedded generator \(\Delta_{ij}\in\mathfrak{so}(n)\). For \(n>2\), the Lie subalgebra generated by \(\{\Delta_{ij}\}\) alone is one-dimensional (the abelian span of \(\Delta_{ij}\)); bracket closure to the \emph{entire} \(\mathfrak{so}(n)\) still requires a suitable seed subalgebra and an \(n\)-dependent certificate. The \(G_2\cup\{\Delta\}\) package of Section~\ref{sec:closure} is specific to the octonionic \(8\)-dimensional carrier and should not be read as a black-box substitute for that data when \(n\neq 8\).
...
For reproducibility, the same exact-rational symbolic pipeline used for the \(\mathfrak{so}(8)\) certificate is mirrored in a toy script (\path{scripts/generate_symbolic_so4_certificate.py}) writing \path{artifacts/so4_symbolic_certificate.json} (paths relative to \PaperArtifactBundleDir/; archive \PaperArtifactBundleZip{} on \PaperZenodoDeposit).
...
\bibitem{hqiv-lean}
HQIV-LEAN repository, \url{https://github.com/disregardfiat/hqiv-lean}, 2026; reproducibility archive \PaperArtifactBundleZip{} (tree rooted at \PaperArtifactBundleDir/; see \path{MANIFEST.json}) is deposited with this manuscript on Zenodo, \PaperZenodoDeposit.
\bibitem{brodie2026}
