...
/-- The algebra-first seed's H block is definitionally the same Cayley-Dickson block used by
`modifiedMaxwellHMatrix`. This keeps the new ladder anchored to the existing EM/H language. -/
theorem modifiedMaxwellHMatrix_eq_algebraicMaxwellHBlock (n : ℕ) :
    modifiedMaxwellHMatrix n = Hqiv.algebraicMaxwellHBlock n := by
  rfl

/-- Heavy-side bridge: the old EM/H block statement can be read directly as the new algebra-first
Maxwell seed block. -/
theorem em_magnetic_block_is_algebraicMaxwellHBlock (k : Fin 28) :
    quadrantBottomRight (Hqiv.so8Generator k) = Hqiv.algebraicMaxwellHBlock k.val := by
  rw [em_magnetic_block_is_modified_maxwell_H, modifiedMaxwellHMatrix_eq_algebraicMaxwellHBlock]
