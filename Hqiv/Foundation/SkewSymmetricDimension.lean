...
/-!
# The dimension of `so(n)` from the standard skew-symmetric basis

This module grounds the carrier count `dim so(8) = 28` in honest linear algebra,
replacing any reliance on a numerically-orthonormalized floating-point generator
matrix (whose exact-rational orthogonality relation is only approximate).

The argument is fully general in `n` ("works for `so(n)`"):

* `skewMatrices n` is the submodule of real `n × n` skew-symmetric matrices.
* `skewGen i j = single i j 1 - single j i 1` is the standard generator attached
  to an ordered pair `i < j`.
* These generators are **linearly independent** (`skewGen_linearIndependent`), by
  reading off the `(i,j)` matrix entry — an exact computation, no orthonormality.
* They **span** the skew submodule (`skew_repr`), so they form a basis
  (`skewBasis`), giving `finrank ℝ (skewMatrices n) = #{(i,j) : i < j}`.

The `SO(4)` instance (`finrank = 6`) is the worked low-dimensional analogue;
`SO(8)` (`finrank = 28`) recovers the HQIV carrier count, matched to the
Foundation-derived `soDim` in `CarrierBudget`.
-/
...
/-- The submodule of real `n × n` skew-symmetric matrices (`Aᵀ = -A`). -/
...
/-- The standard skew generator attached to a pair `(i, j)`. -/
...
/-- Every standard generator is skew-symmetric. -/
...
/-- **Entry extraction.** Evaluating a coefficient combination of skew generators
at the matrix entry of an ordered pair `p` returns exactly the coefficient `c p`.
This is the load-bearing exact computation behind both independence and spanning. -/
...
/-- **Spanning / representation.** Every skew-symmetric matrix is the combination
of standard generators with its own upper-triangle entries as coefficients. -/
lemma skew_repr {A : Matrix (Fin n) (Fin n) ℝ} (hA : Aᵀ = -A) :
  ...
  · -- x < y: only the (x,y) generator contributes, with coefficient A x y
    ...
    · intro q _ hq
      ...
      simp [h1, h2]
    · intro h; exact absurd (Finset.mem_univ _) h
  · -- x = y: skew diagonal vanishes and no generator contributes
    subst heq
    ...
    have h2 : ¬ (q.1.2 = x ∧ q.1.1 = x) := by
      rintro ⟨e1, e2⟩; have hlt := q.2; rw [e1, e2] at hlt; exact lt_irrefl x hlt
    simp [h1, h2]
  · -- y < x: the (y,x) generator contributes with coefficient A y x = -(A x y)
...
variable (n) in
/-- **The standard skew basis** of `so(n)`. -/
...
variable (n) in
/-- **Dimension of `so(n)`** equals the number of ordered index pairs `i < j`. -/
...
/-- Worked low-dimensional analogue: `dim so(4) = 6`. -/
theorem finrank_so4 : Module.finrank ℝ (skewMatrices 4) = 6 := by
  rw [finrank_skewMatrices]; decide

/-- The HQIV carrier: `dim so(8) = 28`, from genuine linear algebra. -/
theorem finrank_so8 : Module.finrank ℝ (skewMatrices 8) = 28 := by
  rw [finrank_skewMatrices]; decide

/-- **Bridge to the Foundation combinatorial count.** The genuine linear-algebra
dimension of the carrier's rotation algebra equals the derived `soDim`
(`= n(n−1)/2`) from `CarrierBudget`. So the carrier count `28` is grounded in real
skew-symmetric matrices, not in a floating-point orthonormal generator table. -/
theorem finrank_skew_carrier_eq_soDim :
    Module.finrank ℝ (skewMatrices carrierMultiplicity) = soDim carrierMultiplicity := by
  rw [carrierMultiplicity_eq_eight, finrank_so8]
  exact soDim_eight.symm

/-! ### Genuine Lie closure of `so(n)`

The matrix commutator preserves skew-symmetry, so the standard skew basis is closed
under brackets — and the closing coefficients are *free* from the spanning lemma
`skew_repr`. This yields an axiom-free analogue of the symbolic `so(8)` closure
interface (antisymmetry + bracket-closure + linear independence), with **no**
dependence on any floating-point orthonormality relation. -/
...
variable (n) in
/-- **Genuine, axiom-free `so(n)` Lie-closure certificate** for the standard skew
basis, mirroring the shape of the symbolic interface `so8_closure_theorem_symbolic`
but with zero `axiom`/`sorry` and no floating-point orthonormality:
...
/-- The carrier instance: a genuine axiom-free `so(8)` Lie-closure certificate. -/
