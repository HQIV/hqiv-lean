...
/-!
# Molecular dipole from the balanced frame

The molecular dipole is the vector sum of bond dipoles sitting on the VSEPR balanced frame.  Because
the frame is symmetric with vector sum zero (`VSEPRFromBalance`), the magnitude is controlled by the
resultant of the *bond* subset `S` of the `d` domains.  The Gram identity of the frame gives a closed
form for that resultant:

`‖∑_{i∈S} v̂ᵢ‖² = |S| + |S|(|S|−1)·c`, and with the balanced cosine `c = −1/(d−1)` this is
`|S|(d−|S|)/(d−1)`.  When every domain is a bond (`|S| = d`) it vanishes — CO₂, CH₄, BF₃, SF₆ are
nonpolar by the *same* balance that fixes their angles.  Lone pairs (`|S| < d`) break the cancellation
and give H₂O, NH₃, SO₂ their dipoles.
-/

namespace Hqiv.QuantumChemistry.MolecularDipoleBalance
...
end Hqiv.QuantumChemistry.MolecularDipoleBalance
