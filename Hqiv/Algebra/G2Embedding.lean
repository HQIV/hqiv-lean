...
/-!
## HQIV holonomy chart: six-pack + \(e_1\)–\(e_4\) pair

**Physical picture.** SO(8) is generated by G₂ derivations `[L(e_i),L(e_j)]` (“two spheres
rolling” on the imaginary octonions) plus Δ in the preferred \((e_1,e_7)\) U(1) suture.
The **six-pack** indexes the middle commutators through \(e_2\):
`[e_2,e_3] … [e_2,e_7], [e_3,e_4]`. Full \(\mathfrak{so}(8) = \mathrm{Lie}(G_2 \cup \{\Delta\})\)
is proved in `G2DeltaGeneratedLie.g2DeltaGeneratedLie_eq_so8LieSubalgebra`.
-/

def G2Generators : Set (Matrix (Fin 8) (Fin 8) ℝ) :=
  Set.range g2Generator

def G2UnionDelta : Set (Matrix (Fin 8) (Fin 8) ℝ) :=
  G2Generators ∪ {Hqiv.phaseLiftDelta}

/-- Six middle-sector commutators (indices `6 … 11` in the 14-element G₂ chart). -/
def g2SixPackMiddle : Finset (Fin 14) :=
  {6, 7, 8, 9, 10, 11}

theorem card_g2SixPackMiddle : g2SixPackMiddle.card = 6 := by
  native_decide

theorem g2SixPack_labels :
    g2Generator 6 = Hqiv.g2_comm_23 ∧
    g2Generator 7 = Hqiv.g2_comm_24 ∧
    g2Generator 8 = Hqiv.g2_comm_25 ∧
    g2Generator 9 = Hqiv.g2_comm_26 ∧
    g2Generator 10 = Hqiv.g2_comm_27 ∧
    g2Generator 11 = Hqiv.g2_comm_34 := by
  refine ⟨rfl, rfl, rfl, rfl, rfl, rfl⟩

/-- Two \(e_1\)–\(e_4\) rotations: `[e_1,e_4]`, `[e_2,e_4]`. -/
def g2E1E4Pair : Finset (Fin 14) :=
  {2, 7}

theorem g2E1E4Pair_labels :
    g2Generator 2 = Hqiv.g2_comm_14 ∧
    g2Generator 7 = Hqiv.g2_comm_24 := by
  constructor <;> rfl

/-!
## Recoverable holonomy field (linear G₂ + Δ chart)
-/

/-- Coefficients on the 14 G₂ commutators plus the Δ suture direction. -/
structure G2DeltaHolonomyCoeffs where
  g2 : Fin 14 → ℝ
  delta : ℝ

noncomputable def G2DeltaHolonomyCoeffs.toMatrix (c : G2DeltaHolonomyCoeffs) :
    Matrix (Fin 8) (Fin 8) ℝ :=
  ∑ k : Fin 14, c.g2 k • g2Generator k + c.delta • Hqiv.phaseLiftDelta

private theorem skew_smul_g2 (r : ℝ) (k : Fin 14) :
    r • g2Generator k + (r • g2Generator k)ᵀ = 0 := by
  ext i j
  simp [Matrix.add_apply, Matrix.smul_apply, Matrix.transpose_apply]
  have hij : g2Generator k i j + g2Generator k j i = 0 := by
    simpa [Matrix.add_apply, Matrix.transpose_apply, Matrix.zero_apply] using
      congrArg (fun M => M i j) (g2_in_so8 k)
  rw [← mul_add, hij, mul_zero]

private theorem skew_smul_delta (r : ℝ) :
    r • Hqiv.phaseLiftDelta + (r • Hqiv.phaseLiftDelta)ᵀ = 0 := by
  ext i j
  simp [Matrix.add_apply, Matrix.smul_apply, Matrix.transpose_apply]
  rw [← mul_add, Hqiv.phaseLiftDelta_antisymm i j, mul_zero]

private theorem skew_finset_sum {ι : Type} [Fintype ι] (f : ι → Matrix (Fin 8) (Fin 8) ℝ)
    (h : ∀ i, f i + (f i)ᵀ = 0) :
    (∑ i, f i) + (∑ i, f i)ᵀ = 0 := by
  rw [Matrix.transpose_sum, ← Finset.sum_add_distrib]
  simp only [h, Finset.sum_const_zero]

private theorem skew_add (A B : Matrix (Fin 8) (Fin 8) ℝ) (hA : A + Aᵀ = 0) (hB : B + Bᵀ = 0) :
    A + B + (A + B)ᵀ = 0 := by
  rw [Matrix.transpose_add]
  have hsum : A + B + (Aᵀ + Bᵀ) = (A + Aᵀ) + (B + Bᵀ) := by abel
  rw [hsum, hA, hB, add_zero]

/-- A G₂+Δ linear combination is antisymmetric (hence in the matrix model of \(\mathfrak{so}(8)\)). -/
theorem G2DeltaHolonomyCoeffs.toMatrix_skew (c : G2DeltaHolonomyCoeffs) :
    c.toMatrix + c.toMatrixᵀ = 0 := by
  unfold G2DeltaHolonomyCoeffs.toMatrix
  exact skew_add _ _ (skew_finset_sum _ fun k => skew_smul_g2 (c.g2 k) k) (skew_smul_delta c.delta)
