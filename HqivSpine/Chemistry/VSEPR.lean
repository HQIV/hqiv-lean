...
/-!
# `HqivSpine.Chemistry.VSEPR` — bond angles from equilibrium, not a chemistry rule
...
* **balance** — the centre is in equilibrium, so the contact directions carry no net flux,
  `∑ᵢ vᵢ = 0` (Kirchhoff's node law / momentum balance);
* **symmetry** — equivalent contacts share one pairwise inner product `c`.

Then `0 = ‖∑ᵢ vᵢ‖² = d·(1 + (d−1)c)`, so `c = −1/(d−1)`. Nothing about chemistry enters — only
equilibrium of unit vectors.

Mathlib-only; no legacy `Hqiv.*`, no `sorry`, no new `axiom`, no `native_decide`.
-/

namespace HqivSpine.Chemistry.VSEPR
...
`d·(1 + (d−1)·c) = 0`, where `c` is the common pairwise inner product (the Gram trace of a
zero-sum symmetric frame). -/
theorem balanced_unit_contacts_identity
    ...
    (d : ℝ) * (1 + ((d : ℝ) - 1) * c) = 0 := by
  have hzero : (⟪(∑ i, v i), (∑ j, v j)⟫ : ℝ) = 0 := by rw [hsum]; simp
...
pairwise cosine is exactly `−1/(d−1)` — the steric-domain angle, from equilibrium alone. -/
