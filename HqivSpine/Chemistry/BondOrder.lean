...
/-!
# `HqivSpine.Chemistry.BondOrder` — bond order as the saturated 2×2 coupling

Treat each atom of a bond as its own system offering a per-bond capacity: `a` from one side, `c`
from the other. A shared bond is the off-diagonal coupling `b` in the 2×2 Gram matrix
`[[a, b], [b, c]]` (diagonal = each system's own offer, off-diagonal = shared pairs).
...
saturates the bound**: `b = √(a·c)` makes `M` singular (rank 1) — one fully-shared eigen-channel =
one shared pair. So the geometric-mean bond order is *derived* as the maximal coherent coupling.

Mathlib-only; no legacy `Hqiv.*`, no `sorry`, no new `axiom`, no `native_decide`.
-/

namespace HqivSpine.Chemistry.BondOrder
...
/-- **Homonuclear collapse.** Equal offers ⇒ bond order is the offer itself (`√(a·a)=a`). -/
theorem geometric_mean_homonuclear (a : ℝ) (ha : 0 ≤ a) :
    Real.sqrt (a * a) = a := Real.sqrt_mul_self ha

/-- The saturated channel has eigenvalues `a+c` (bonding) and `0` (antibonding): trace `a+c`, the
two offers fully pooled into one shared pair. -/
theorem saturated_eigen_sum (a c : ℝ) : (a + c) + 0 = a + c := by ring

end HqivSpine.Chemistry.BondOrder
