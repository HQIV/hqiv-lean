...
/-!
# `HqivSpine.Topology.NullLatticeComplex` — finite null-lattice 3-complexes

Topology as an **output** of the discrete causal/curvature programme. A finite closed
3-complex is built from the `3+1` null-shell combinatorics: shell `m` carries
`latticeSimplexCount m = shellNumer m = (m+2)(m+1)` stars-and-bars tags (the spine's own
`Physics.Shell.latticeSimplexCount`), and a `Discrete3Complex` records vertices/edges/
triangles/tetrahedra with a closedness sketch.

The central object is the **signed shell-budget mismatch**
`shellBudgetMismatch M m = (#vertices at shell m) − latticeSimplexCount m`, which vanishes
exactly on the quadratic null-shell growth law. Main results:

* `not_quadratic_null_shell_growth` — **no finite complex** realises the growth law on *all*
  shells (the shell above the top occupied one is empty but has positive budget);
* the finite-horizon law `QuadraticNullShellGrowthOnHorizon M n` and the reference template
  `S3NullReference n` (one vertex per tag on shells `0…n`) which realises it
  (`S3NullReference_quadratic_on_horizon`, `S3NullReference_shell_budget_zero`);
* `quadraticOnHorizon_is_S3NullReference` — on-horizon quadratic growth pins the vertex set
  to the reference;
* Euler-characteristic readouts: a vertex-only complex has `χ = |V|`, so the reference is
  **not** combinatorially spherical (`S3NullReference_not_combinatorially_spherical`), and a
  positive shell excess obstructs `χ = 0` (`shell_budget_excess_obstructs_chi_zero`).

Rebased entirely on the spine's `latticeSimplexCount`; Mathlib + spine only, no legacy
`Hqiv.*`, no `sorry`, no new `axiom`, no `native_decide`.
-/
...
/-- A finite closed 3-complex: 0–3 cells plus a closedness sketch. -/
structure Discrete3Complex (α : Type) where
  /-- Vertex labels. -/
  vertices : Finset α
  /-- 1-cells (unordered pairs). -/
  edges : Finset (UndirectedEdge α)
  /-- 2-cells: oriented triangles as ordered 3-tuples. -/
...
/-- Combinatorial Euler characteristic `χ = |V| − |E| + |F| − |T|`. -/
...
/-- Idealised growth on **all** shells `m : ℕ` (infinite-horizon limit only). Finite
complexes cannot satisfy this — see `not_quadratic_null_shell_growth`. -/
...
/-- **Finite-horizon law (primary).** Quadratic null-shell budget on shells `0…n`. -/
...
/-- Alias emphasising the finite-complex / holonomy-bridge use case. -/
...
theorem not_quadratic_null_shell_growth (M : Discrete3Complex NullShellVertex) :
  ...
  · have hz : Discrete3Complex.vertexCountAtShell M (maxVertexShell M + 1) = 0 :=
...
/-- Local shell defect ↔ failure of quadratic null-shell growth. -/
...
/-- A fully triangulated closed 3-manifold has `χ = 0` (e.g. `χ(S³)=0`). -/
def IsCombinatoriallySpherical {α : Type} (M : Discrete3Complex α) : Prop :=
  M.eulerCharacteristic = 0

/-- Combinatorial equivalence (vertex-set bijection). -/
...
/-- Reference complex at horizon index `n`: one vertex per tag on each shell `0…n`. -/
...
/-- On shells inside the horizon, the reference realises the quadratic budget exactly. -/
...
theorem S3NullReference_vertices_card_pos (n : ℕ) :
  ...
  exact lt_of_lt_of_le (latticeSimplexCount_pos 0)
    (Finset.single_le_sum (fun _ _ => Nat.zero_le _) hmem)

/-- Matches the null-lattice vertex template at horizon `n`. -/
...
theorem quadraticOnHorizon_vertices_eq_S3NullReference
  ...
  constructor
  · intro hv
    have hshell : v.shell ≤ n := le_trans (vertex_shell_le_maxVertexShell hv) hmax
    have hfilter :
...
/-! ## Local detection of shell defects via Euler characteristic -/
...
/-- The vertex-only reference template has `χ = |V| > 0`, so it is not combinatorially
spherical. -/
...
/-- **A shell-budget defect obstructs quadratic null-shell growth.** -/
...
/-- **A positive shell excess forces `χ ≠ 0`** for a vertex-only complex. -/
...
/-- The reference vertex template is equivalent to itself. -/
theorem S3NullReference_is_template (n : ℕ) :
    IsS3NullVertexTemplate (S3NullReference n) n :=
