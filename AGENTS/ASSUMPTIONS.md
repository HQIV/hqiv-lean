...
**Patch ontology (agent contract):** [PATCH_ONTOLOGY.md](./PATCH_ONTOLOGY.md) — accessible patch as observable universe, continuum as readout not foundation, patch-closed “completeness,” **gauge consistency discharged vs continuum-only** (§2b), simulation aliasing. Read before conflating `main.tex` “complete GUT” language with proved continuum QFT.
...
- **No external inputs in HQIV Lab / chemistry prediction paths:** W4-17, GMTKN55, NIST, CRC, PDG, CODATA, AMU tables, bond-length tables, bond-angle tables, crystallographic constants, density tables, hydration shells, mobility tables, and fitted potentials are **quarantined comparison data only**. They must not be used to build `MoleculeSpec`, contact geometry, packing, phase, binding, electrolyte, conductivity, or material-response formulas. Prediction paths may use only HQIV-derived atomic/nuclear/electronic readouts, TUFT/nested-wavefunction bond geometry, α = 3/5, γ = 2/5, and the single active `proton_lockin` mass witness at `referenceM = 4`. Exact unit conversions may be used solely to report outputs in conventional units.
...
## 2b. Gauge consistency: discharged at patch level vs continuum-only

Mirrors [PATCH_ONTOLOGY.md](./PATCH_ONTOLOGY.md) § “Gauge consistency.” Agents must **not** collapse these rows into “HQIV proves continuum QFT is anomaly- and instanton-free.”

| Obligation | Patch / finite status | Lean anchor | Still open (continuum-only) |
|------------|----------------------|-------------|------------------------------|
...
| Instanton / Pontryagin / first-Chern / \(U(1)\) winding on patch | **Discharged** (single sector) | `Hqiv/QuantumMechanics/PatchTopologicalObstruction` | BPST sectors, \(\theta\)-vacua on smooth bundles |
| Abelian patch commutator / microcausality hooks | **Proved** | `PatchQFTBridge`, `Chapter07_PatchQFT` | Full smooth non-abelian holonomy |
| Strong-color \(\mathfrak{su}(3)\) chart Lie law \(f^{abc}\) | **Proved** (optional `HQIVStrongColorSu3Certificate` build) | `StrongColorSu3LieChartLaw` | Dynamical YM / Clay mass gap |
| CKM/PMNS from Fano cycle overlaps | **Scaffold only** | `HopfShellBeltramiMassBridge`, `ContinuousXiCoupling`; programme: [CKM_PMNS_FANO_OVERLAP.md](./CKM_PMNS_FANO_OVERLAP.md) | TUFT \(H^*(CP^4)\) intersection form; PDG matrix fit |
| Exact CCR \([A,B]=I\) on fixed finite dim | **Obstruction proved** | `CCRFiniteDimObstruction` | Global \(L^2\) CCR (not HQIV ontology) |

**Papers:** shared reader contract in `papers/include/patch_theory_messaging.tex` (included by Lean-aligned `.tex` manuscripts).
...
- **Strong-color SU(3) certificate stack (optional `HQIVStrongColorSu3Certificate`):** `StrongColorSu3fStructureSimp` and `StrongColorSu3LieChartLaw` are **generated Lean** from `scripts/gen_strong_color_su3_f_simp.py` and `scripts/gen_strong_color_su3_lie_chart_law.py`. The Lie-chart file proves the global `su(3)` bracket identity on the abstract `3×3` chart by **64 explicit matrix cases** (not a single abstract Lie-algebra tactic). Trust boundary is “regen scripts + `lake build HQIVStrongColorSu3Certificate` agree”; default `HQIVLEAN` does not import this cone. **Claim discipline:** treat `colorHalfGellMannFull_lieBracket_eq_I_smul_f_sum` as a **proved finite-chart Lie law**, not as dynamical non-abelian QCD or confinement (see §2b, [PATCH_ONTOLOGY.md](./PATCH_ONTOLOGY.md)).
- **`so8Generator` matrices:** Originate from the same pipeline as `matrices.py` / project scripts (see repo docs).
- **Spinor monomial Gram certificate (now proved, fully axiom-clean):** `CliffordCl06SixSpinorMonomialMatrixData.lean` proves the closed form `spinorMonomialGramColumns_eq_one` (`W = I₆₄`), from which `eight_dvd_spinorMonomialGramFrobSum` and `spinorMonomialGramColumns_det_ne_zero` are **theorems** depending only on `[propext, Classical.choice, Quot.sound]` (verified by `#print axioms`). The former external-script axioms (`spinorMonomialGramColumnsZMod101_det` and the divisibility axiom), the `ZMod 101` machinery, **and** any `native_decide`/`Lean.ofReduceBool` dependence have been removed. The proof uses the Kronecker mixed-product property to factorize the `8×8` Frobenius pairing into three `2×2` pairings (`spinorKron3Z_mul`, `…_eq_kron`, `spinorMonomialGramFrobSum_eq_frob2_prod`); the residual `64×64` `2×2` check (`frob2_prod_monoSlot'`) is closed by plain kernel `decide` after `maskFinset_sort_eq` rewrites `Finset.sort` to a kernel-reducible `List.finRange` filter (`decide` adds no trust axioms; module build ≈ 56 s).
...
If module A imports module B, **all** definitions and axioms of B (and Mathlib transitive closure) are in force. The root `Targets/HQIVLEAN.lean` lists the **intended** high-level import graph; individual features may compile under smaller `lake` targets — see `lakefile.toml`.
...
| **L5 — Zeta / modular / analytic** | `OctonionicZeta`, `ModularThetaBridgeScaffold`, `ThetaCompletedLFunctionalScaffold`, `MulModBSDCompletedLFunctionalScaffold` | Convergence, residue partitions, trivial-character Dirichlet hooks, `r₈` bounds; **mul-mod BSD** coeff `\|a_n\|≤1` + `mulModBSDLSeries` on `\Re s>1` | **`ThetaZ8ModularRealization.coeff_eq`** (classical θ = `r₈`); **weight-4** completed L for HQIV θ coeffs; **`MulModBSDLObjectHypothesis`** (weight-2 BSD-shaped completed L) |
| **L6 — Continuum / QFT bridges** | `LightConeMaxwellQFTBridge`, `HorizonLimitedRenormLocality`, `ContinuumManyBodyQFTScaffold`, `AnomalyCancellation`, `PatchTopologicalObstruction` | Mode budgets, shell↔harmonic limits, microcausality scaffolds, cluster kernels; **finite SM anomaly traces**; **patch topological-sector discharge** | **`HorizonContinuumAxioms*`** records; full renormalised QFT **not** a goal; **CKM/PMNS from Fano overlaps** scaffold only (§2b) |
...
- **Zeta:** any **partial** result on `ThetaZ8LSeriesCoeff` or Dirichlet abscissa (already have bounds); **mul-mod BSD channel** (`MulModBSDLSeriesScaffold`) now has half-plane holomorphy; full modular identification is **hard** (classical θ / Hecke theory).
