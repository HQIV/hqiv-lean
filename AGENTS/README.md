...
- **Strong color (`su(3)` triplet chart):** `Hqiv/Physics/QuarkColorCarrierGaugeScaffold.lean`, `StrongColorSu3ChartClosure.lean` (Gell–Mann / `f^{abc}` defs; matrix entries use explicit `Complex.I`), `StrongColorCarrierClosure.lean` (`colorGellMannEmbed`, chart–carrier Lie bridge); optional certificate `lake build HQIVStrongColorSu3Certificate`: `@[simp]` `f^{abc}` table (`scripts/gen_strong_color_su3_f_simp.py`) **and** proved global chart Lie law `colorHalfGellMannFull_lieBracket_eq_I_smul_f_sum` in `StrongColorSu3LieChartLaw.lean` (`scripts/gen_strong_color_su3_lie_chart_law.py`)
...
When you add a **user-facing** formal claim that agents should rely on, add a one-line entry under the right heading in `THEOREMS.md`. When you add **new trust assumptions** (script data, `sorry`, or explicit `Prop` bundles), document them in `ASSUMPTIONS.md`. When **patch / continuum / completeness** vocabulary changes in papers or simulators, update [PATCH_ONTOLOGY.md](./PATCH_ONTOLOGY.md) and keep `papers/include/patch_theory_messaging.tex` in sync for LaTeX.

**Agents:** if `main.tex` or a benchmark README contradicts `PATCH_ONTOLOGY.md`, treat the AGENTS contract as authoritative for in-repo work and file paper edits separately.
